...
class PeleeNet3(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
        ...
        super(PeleeNet3, self).__init__()
        original_model = PeleeNet2(num_classes=1999)
        #print(original_model)
        
        # Everything except the last linear layer & Avg_Pool
...
class PeleeNet2(nn.Module):

    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
                 num_init_features=32, bottleneck_width=[1, 2, 4, 4], drop_rate=0.05, num_classes=1000):
        super(PeleeNet2, self).__init__()
        original_model = PeleeNet()
        #print(original_model)
        
        # Everything except the last linear layer & Avg_Pool
        self.features = nn.Sequential(*list(original_model.children())[:-1])
        self.features = nn.Sequential(*list(original_model.children())[0][0:11])
        num_features = 704
        self.features.add_module('transition4', BasicConv2d(
            num_features, num_features, kernel_size=1, stride=1, padding=0))
        #print(self.features[11])
        m = self.features[11]
